...
def detect_blobs(image, debug=False, debug_level=1):
    ...
    im2, contours, h = cv2.findContours(tresholded_im, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def resized_image(im, how_much_smaller=1):
    ...
    if len(im.shape) > 2:
        ...
        rows, cols = im.shape
    return cv2.resize(im, ( int(cols / how_much_smaller), int(rows / how_much_smaller))
...
##    cv2.imshow('frame', resized_image(image))
